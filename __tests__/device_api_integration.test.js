...
test("device api upload via js library", async () => {
  await waitForBackend();

  const accessToken = await registerAndLogin();
  const projectId = await createProject(accessToken);
  const { readKey, writeKey } = await enableExternalApi(projectId, accessToken);
  ...
  collector.addDataPoint(baseTime + 20, "Acc", 3.0);
  ...
  expect(dataset.timeSeries).toHaveLength(1);
  expect(dataset.timeSeries[0].length).toBe(3);
});
